function epicConvertTDMSmat()

% ********************************************************************
%  --> Note: Need to change folder from which you are pulling .tdms files.
%  You also need to comment/uncomment correct lines for whether you are
% using a Mac OS X system, or a Windows system. <--
% ********************************************************************
...
oldFolder = cd('volt_data'); % CHANGE FOR INDIVIDUAL USE
currFolder = pwd;

% Get list of all subfolders.
allSubFolders = genpath(currFolder);
...
while true
    switch macFlag
        case 1
            [singleSubFolder, remain] = strtok(remain, ':'); % For Mac OS X
        case 0
            [singleSubFolder, remain] = strtok(remain, ';'); % For Windows
    end
...
tdFolder = cd(oldFolder);

% Process all TDMS files in those folders.
for k = 1 : numberOfFolders
	...
	% Get PNG files.
	filePattern = sprintf('%s/*.tdms', thisFolder);
	...
	if numberOfFiles >= 1
		% Go through all those tdms files.
		for f = 1 : numberOfFiles
			fullFileName = fullfile(thisFolder, baseFileNames(f).name);
			fprintf('     Processing TDMS file %s\n', fullFileName);
            matFileName = epic_convert_tdms_mat(fullFileName);
		end
	else
		fprintf('     Folder %s has no TDMS files in it.\n', thisFolder);
	end
end
...
function matFileName=epic_convert_tdms_mat(fileName,ignoreGroupNames)
...
%                 If not supplied, the user is provided a 'File Open' 
%                 dialog box to navigate to a file.  Can be a cell array
...
%           because LV uses an ugly (timestamp) for the default
...
%-------------------------------------------------------------------------
%Brady Anderson - v1.4 2015-5-7
%BYU_Mechanical Engineering
%Moved ignoreGroup structure to occur earlier in file. Added cell array to
%make it easier to remove data as arrays, rather than having them stuck in
%structures. This cell array is exported as '[fileName]_data.mat',
%containing one variable, 'dataCell'. Changed safeChannelNames to use
%built-in matlab.lang.makeValidName function.
%  --> Note: Need to change name of channels that individual knows were
% used to output the data from LV. <--
%-------------------------------------------------------------------------

if nargin==0
    %Prompt the user for the file
    [fileName,pathName]=uigetfile({'*.tdms','All Files (*.tdms)'},...
                                   'Choose a TDMS File');
    if fileName==0
        return
    end
...
numFileNames = numel(inFileName);
matFileName = cell(numFileNames,1); % pre-allocate cell array

for fnum=1:numFileNames  % Loop through cells of filename
    
    tdmsFileName=inFileName{fnum};
    
    %Perform Conversion
    [convertedData,dataOb.convertVer,d.chanNames,d.groupNames,dataOb.ci]...
        =convertTDMS(0,tdmsFileName);
    ...
    % Changed by Brady Anderson
    % move the ignore group names to outside of the inner FOR loop
    numChanNames = numel(channelNames);
    if ignoreGroupNames      %Drop the group name
        for cnum=1:numChanNames
    ...
    % ************************************************
    % take out data vectors for ease of use
    % return a cell array of data, named dataCell, for ease of use later.
    % Added by Brady Anderson, 6 May 2015
    % Mechanical Engineering_BYU
    
    safeChannelNameValid = matlab.lang.makeValidName(channelNames, ...
                          'ReplacementStyle','delete','Prefix','d');
    safeChannelName = matlab.lang.makeUniqueStrings(safeChannelNameValid);
    
    chanNames = safeChannelNameValid; % cell array of valid names, from 
%                                       'ConvertTDMS'
    chanNamesU = safeChannelName;     % cell array of Unique valid names,
%                                       for creating structure variable
    
    checkT = 0;
    checkS = 0;
    indUnVo = zeros(4,1); % index untitled voltage, default 4 channels in
    chansUsed = cell(1,1);
    chansNotUsed = cell(1,1);
    
    for cnumC = 1:numChanNames
        chField = isempty(convertedData.Data.MeasuredData(cnumC).Data);
%         try chField = chanNames{cnumC}(1:8) == ('Untitled');
%             chNam = chanNames{cnumC}(9); % name Checker
%             chField = [ chNam == ('V');
%                         chNam == ('C');
%                         chNam == ('A');
%                         chNam == ('L') ];%CHANGE FOR INDIVIDUAL USE
            if chField == 0
                checkT = checkT + 1;
                indUnVo(checkT,1) = cnumC;
                chansUsed{checkT,1} = chanNames{cnumC};
            else
                checkS = checkS + 1;
                chansNotUsed{checkS,1} = chanNames{cnumC};
            end
%         catch
% %             dataCell(check,1) = [];
%         end
    ...
    if checkT ~= 0 %|| checkS ~= 0
        numProps = size(convertedData.Data.MeasuredData(cnumC).Property,2);
        for prIt = 1:numProps % property number iterator
            tempProp = ...
               convertedData.Data.MeasuredData(cnumC).Property(prIt).Name;
               % temporary property name, changes iterating through
            
            if strcmp(tempProp, 'wf_increment');
                timeStep = convertedData.Data.MeasuredData(cnumC)...
                    .Property(prIt).Value;
                break
            end
        end
        
        dataCell = cell(checkT,1);
        voltCel = 0;
        for dataTr = 1:length(indUnVo) % true data vectors
            if indUnVo(dataTr) == 0
                break
            else
                voltCel = voltCel + 1;
                dataCell{voltCel,1} = ...
                    convertedData.Data.MeasuredData(indUnVo(dataTr)).Data;
            end
        end
        
%         dataCell(1,:) = [];
    else
        dataCell{1,1} = {'No valid data set'; 'was provided'};
        chansUsed{1,1} = {' No valid data set'};
        timeStep = {'no valid time step'}
    end
    
    if checkS == 0
        chansNotUsed{1,1} = {'No valid data set'};
    end
    ...
    for cnum=1:numel(channelNames)
        ...
        dataOb.(chanNamesU{cnum}).Data= ...
            convertedData.Data.MeasuredData(cnum).Data;
        dataOb.(chanNamesU{cnum}).Total_Samples= ...
            convertedData.Data.MeasuredData(cnum).Total_Samples;
        ...
        for pcnt=1:numel(prop)
            ...
            safePropName = matlab.lang.makeValidName(pName, ...
                                             'ReplacementStyle','delete');
            pValue=prop(pcnt).Value;
            dataOb.(chanNamesU{cnum}).Property.(safePropName)=pValue;
        end
    ...
    % Also added by Brady Anderson
    % saves dataCell cell array as a separate file, by same principle name,
    % just appends '_data' to the name, before the file type identifier
    matName = strcat(fileName(1:end-5), '_data', '.mat');
    save(matName,'dataCell', 'chansUsed', 'chansNotUsed','timeStep');
end
...
function [ConvertedData,ConvertVer,ChanNames,GroupNames,ci]= ...
                                                      convertTDMS(varargin)
...
%
% [ConvertedData,ConvertVer,ChanNames]=
%                                   convertTDMS(SaveConvertedFile,filename)
%
%       Inputs:
%               SaveConvertedFile (required) - Logical flag (true/false)
%                 that determines whether a MAT file is created.  The MAT
%                 file's name is the same as 'filename' except that the
%                 'TDMS' file extension is replaced with 'MAT'.  The MAT
%                 file is saved in the same folder and will overwrite an
%                 existing file without warning.  The MAT file contains 
%                 all the output variables.
%
%               filename (optional) - Filename (fully defined) to be
%                 converted. If not supplied, the user is provided a
%                 'File Open' dialog box to navigate to a file.  Can be a
%                 cell array of files for bulk conversion.
%
%       Outputs:
%               ConvertedData (required) - Structure of all of the data
%                                          objects.
...
%
%
%'ConvertedData' is a structure with 'FileName', 'FileFolder',
%'SegTDMSVerNum', 'NumOfSegments' and 'Data' fields'. The 'Data' field is
% a structure.
%
%'ConvertedData.SegTDMSVerNum' is a vector of the TDMS version number for
% each segment.
%
%'ConvertedData.Data' is a structure with 'Root' and 'MeasuredData' fields.
%
%'ConvertedData.Data.Root' is a structure with 'Name' and 'Property'
% fields.
%The 'Property' field is also a structure; it contains all the specified
% properties (1 entry for each 'Property) for the 'Root' group. For each
%'Property' there are 'Name' and 'Value' fields. To display a list of all
% the property names, input '{ConvertedData.Data.Root.Property.Name}'' 
% in the Command Window.
%
%'ConvertedData.Data.MeasuredData' is a structure containing all the 
% channel/group information. For each index (for example,
% 'ConvertedData.Data.MeasuredData(1)'), there are 'Name', 'Data' and
% 'Property' fields.  The list of channel names can be displayed by typing
% 'ChanNames' in the Command Window.  Similarly, the list of group names 
% can be displayed by typing 'GroupNames' in the Command Window.
%The 'Property' field is also a structure; it contains all the specified
% properties for that index (1 entry in the structure for each 'Property').
% Any LabView waveform attributes ('wf_start_time', 'wf_start_offset',
% 'wf_increment' and 'wf_samples') that may exist are also included in the
% properties. For each 'Property' there are 'Name' and 'Value' fields.  To
% display a list of all the property names, input
% '{ConvertedData.Data.MeasuredData(#).Property.Name}'' in the Command
% Window where '#' is the index of interest.
%
%If you recieve an error that DAQmxRaw data cannot be converted, this is 
% due to the details on parsing this data type not being published by NI.
% The work around is to use a VI that reads in the data file and then
% writes it to a new file.  See: 
...
%-Added support for german umlauts (�,�,�,�,�,�,�) in 
% 'propsName'
%-------------------------------------------------------------------------
...
% included in the output structure.  In order to implement this 
% improvement, I added a 'Property' field as a structure to the
% 'ConvertedData' output structure.
...
for fnum=1:numel(infilename)
    ...
    if ~exist(infilename{fnum},'file')
        e=errordlg(sprintf('File ''%s'' not found.',infilename{fnum}), ...
                           'File Not Found');
        uiwait(e)
        return
    end
    ...
    if fid==-1
        e=errordlg(sprintf('Could not open ''%s''.',FileNameLong), ...
    ...
    % Get the raw data
    ob=getData(fid,channelinfo,SegInfo);  %Returns the objects which have
                                          % data.  See postProcess function
                                          % (appends to all of the objects)
    fclose(fid);
    ...
    if SaveConvertedFile
        ...
        ,MATFileNameShort,exception.identifier, exception.message)
        end
    else
        fprintf('\n\nConversion complete.\n\n')
    end
...
%         SegInfo.SegStartPosn: Absolute Starting Position of Segment in
%             file
%         SegInfo.MetaStartPosn: Absolute Starting Position of Meta Data in
%             file
%         SegInfo.DataStartPosn: Absolute Starting Position of Raw Data in
%             file
...
%         SegInfo.SegBigEndian:  0: Little, 1:Big (numeric, leadin, raw,
%             and meta)
...
while (ftell(fid) ~= eoff)
    ...
    if Ttag==84 && Dtag==68 && Stag==83 && mtag==109
        ...
        % (http://zone.ni.com/devzone/cda/tut/p/id/5696)
        % under the 'Lead In' description on page 2:
        %The next eight bytes (64-bit unsigned integer) describe the length
        % of the remaining segment (overall length of the segment minus
        % length of the lead in). If further segments are appended to the
        % file, this number can be used to locate the starting point of 
        % the following segment. If an application encountered a severe
        % problem while writing to a TDMS file (crash, power outage), all
        % bytes of this integer can be 0xFF. This can only happen to the
        % lastsegment in a file.
        ...
        %Move to the beginning position of the next segment
        status=fseek(fid,CurrPosn,'bof');
        if (status<0)
            warning('file glitch');
            break;
        end
    else
        %TDSm should be the first charaters in a tdms file. If not there,
        % error out to stop hunting.
        fclose(fid);
        error(['Unable to find TDSm tag. This may not be a tdms file, ',...
            'or you forgot to add the .tdms extension to the ',...
            'filename and are reading the wrong file']);
...
while (ftell(fid) ~= eoff)
    ...
    if Ttag==84 && Dtag==68 && Stag==83 && mtag==109
        ...
        % the NI documenation on TDMS files when the contributers first
        % started developing this code. 
        ...
        %             error([sprintf(['\n Seqment %.0f of the above ',...
        %                             'file has interleaved data ',...
        %                             'which is not supported with ',...
        %                             'this function. '],segCnt),...
        %                         'Interleaved Data Not Supported']);
        %             fclose(fid);
        %         end
        
%         if kTocBigEndian
%             error(sprintf(['\n Seqment %.0f of the above file uses ',...
%                            'the big-endian data format which is not ',...
%                            'supported with this function. '],...
%                         segCnt),'Big-Endian Data Format Not Supported');
%             fclose(fid);
%         end
        
        if kTocDaqMxRawData
            error(sprintf(['\n Seqment %.0f of the above file ',...
                           'contains data in the DAQmxRaw NI datatype ',...
                           'format which is not supported with this ',...
                           'function. See help documentation in ',...
                           'convertTDMS.m for how to fix this. '],...
                      segCnt),'DAQmxRawData Format Not Supported');
            fclose(fid);
        end
        ...
        if ~ismember(vernum,[4712,4713])
            error(sprintf(['\n Seqment %.0f of the above file used ',...
                           'LabView TDMS file format version %.0f ',...
                           'which is not supported with this function ',...
                           '(%s.m).'],...
                      segCnt,vernum),'TDMS File Format Not Supported');
            fclose(fid);
        end
        ...
        % of the remaining segment (overall length of the segment minus 
        % length of the lead in). If further segments are appended to the 
        % file, this number can be used to locate the starting point of the
        % following segment. If an application encountered a severe problem
        % while writing to a TDMS file (crash, power outage), all bytes of
        % this integer can be 0xFF. This can only happen to the last
        % segment in a file.
...
%       data for the segment.  Add the rawdata offset to get to this
%       object's start point.
...
%       for this object
% multiplier: As part of LV's optimization, it will appenend raw data 
%             writes to the TDMS file when meta data does not change, but
%             forget to update the nValues.  multiplier is the number of
%             times the base pattern repeats.
% skip: When multiple raw data writes are appended, this is the number of
%       bytes between the end of a channel in one block to the begining of
%       that channel in the next block.
...
for segCnt=1:NumOfSeg
    ...
    % +28 bytes: TDSm (4) + Toc (4) + segVer# (4) + segLength (8) + ...
    % metaLength (8)
    ...
    if (kTocNewObjectList==0)
        %Get a list of the objects/channels to loop through
        fnm=fieldnames(index);
        for kk=1:length(fnm)
            ccnt=index.(fnm{kk}).rawdatacount;
            %If there is raw data info in the previous segement, copy it
            %  into the new segement
            if (ccnt>0)
                if (index.(fnm{kk}).index(ccnt)==segCnt-1)
                    ccnt=ccnt+1;
                    index.(fnm{kk}).rawdatacount=ccnt;
                    index.(fnm{kk}).datastartindex(ccnt)=...
                                             SegInfo.DataStartPosn(segCnt);
                    index.(fnm{kk}).arrayDim(ccnt)=...
                                          index.(fnm{kk}).arrayDim(ccnt-1);
                    index.(fnm{kk}).nValues(ccnt)=...
                                           index.(fnm{kk}).nValues(ccnt-1);
                    index.(fnm{kk}).byteSize(ccnt)=...
                                          index.(fnm{kk}).byteSize(ccnt-1);
                    index.(fnm{kk}).index(ccnt)=segCnt;
                    index.(fnm{kk}).rawdataoffset(ccnt)=...
                                     index.(fnm{kk}).rawdataoffset(ccnt-1);
    ...
    if kTocMetaData
        ...
        for q=1:numObjInSeg
            ...
            obLength=fread(fid,1,'uint32',kTocEndian);
            %Get the objects name
            ObjName=convertToText(fread(fid,obLength,'uint8','l'))';
            ...
            if strcmp(ObjName,'/')
                ...
                if strcmpi(long_obname(1),'/') || ...
                                                strcmpi(long_obname(1),'\')
                    long_obname(1)=[];
                end
            end
            newob=0;
            %Create object's name.  Use a generic field name to avoid
            % issues with strings that are too long and/or
            % characters that cannot be used in MATLAB variable names.
            % The actual channel name is retained for the final output
            % structure.
            if exist('ObjNameList','var')
                %Check to see if the object already exists
                NameIndex=find(strcmpi({ObjNameList.LongName},...
                               long_obname)==1,1,'first');
                if isempty(NameIndex)
                    newob=1;
                    %It does not exist, so create the generic name field
                    % name
                    ObjNameList(end+1).FieldName=sprintf('Object%.0f',...
                                                    numel(ObjNameList)+1);
                    ObjNameList(end).LongName=long_obname;
                    NameIndex=numel(ObjNameList);
                end
            else
                %No objects exist, so create the first one using a generic
                % name field name.
            ...
            if rawdataindex==0
                if segCnt==0    %Unclear why this is here, segCnt should
                                % always be >=1, unless there is a
                                % significant error 
                    e=errordlg(sprintf(['Seqment %.0f within ''%s'...
                                        ' has ''rawdataindex' ...
                                        ' value of 0 (%s.m).'],segCnt,...
                        TDMSFileNameShort,mfilename),...
                        'Incorrect ''rawdataindex''');
                    uiwait(e)
                end
                if kTocRawData
                    if (kTocNewObjectList)
                    ...
                    index.(obname).datastartindex(ccnt)=...
                                             SegInfo.DataStartPosn(segCnt);
                    index.(obname).arrayDim(ccnt)=...
                                           index.(obname).arrayDim(ccnt-1);
                    index.(obname).nValues(ccnt)=...
                                            index.(obname).nValues(ccnt-1);
                    index.(obname).byteSize(ccnt)=...
                                           index.(obname).byteSize(ccnt-1);
            ...
            elseif rawdataindex+1==2^32
                ...
                if strcmpi(index.(obname).long_name,'Root')
                    ...
                    %Need to account for the case where an object (besides
                    % the 'root') is added that has no data but reports
                    % using previous.
                    if newob
                        index.(obname).rawdataindex=0;
                    else
...
% was copying the previous segments info instead of just setting it to 
% default values).
%                             index.(obname).rawdatacount=ccnt;
%                             index.(obname).datastartindex(ccnt)=...
%                                            SegInfo.DataStartPosn(segCnt);
...
%                             index.(obname).index(ccnt)=segCnt;
                            ...
                            SegInfo.NumChan(segCnt)=...
                                                 SegInfo.NumChan(segCnt)+1;
                        end
                    end
                end
            else
                ...
                if (index.(obname).dataType~=32)
                    index.(obname).datasize=...
                                      getDataSize(index.(obname).dataType);
                end
                index.(obname).arrayDim(ccnt)=...
                                          fread(fid,1,'uint32',kTocEndian);
                index.(obname).nValues(ccnt)=...
                                          fread(fid,1,'uint64',kTocEndian);
                index.(obname).index(ccnt)=segCnt;
                SegInfo.NumChan(segCnt)=SegInfo.NumChan(segCnt)+1;
                if index.(obname).dataType==32
            ...
            if numProps>0
                ...
                for p=1:numProps
                    ...
                    switch 1
                        ...
                        case 2
                            PropName=fread(fid,propNameLength,...
                                              'uint8=>char','l')';
                        otherwise
                    end
                    propsDataType=fread(fid,1,'uint32',kTocEndian);
                    
                    %Create property's name.  Use a generic field name to 
                    % avoid issues with strings that are too long and/or
                    % characters that cannot be used in MATLAB variable
                    % names.  The actual property name is retained for the
                    % final output structure.
                    if exist('PropertyInfo','var')
                        %Check to see if the property already exists for
                        % this object.  Need to get the existing
                        % 'PropertyInfo' structure for this object. The 
                        % 'PropertyInfo' structure is not necessarily the
                        % same for every object in the data file.
                        PropIndex=find(strcmpi({PropertyInfo.Name},...
                                                               PropName));
                        if isempty(PropIndex)
                            %Is does not exist, so create the generic name
                            % field name
                            propExists=false;
                        ...
                        %No properties exist for this object, so create the
                        % first one using a generic name field name.
                        propExists=false;
                        PropIndex=p;
                        propsName=sprintf('Property%.0f',PropIndex);
                        PropertyInfo(PropIndex).Name=PropName;
                    ...
                    %Get the number of samples already found and in the
                    % object
                    if dataExists
                    ...
                    if propsDataType==32 %String data type
                        ...
                        propsValue=convertToText(fread(fid,...
                              propsValueLength,'uint8=>char',kTocEndian))';
                        if propExists
                            ...
                            index.(obname).(propsName).value{cnt}=...
                                                                propsValue;
                            index.(obname).(propsName).samples(cnt)=nsamps;
                        else
                            if strcmp(index.(obname).long_name,'Root')
                                %Header data
                                index.(obname).(propsName).name=...
                                                  index.(obname).long_name;
                                index.(obname).(propsName).value=...
                                                              {propsValue};
                                index.(obname).(propsName).cnt=1;
                            else
                                index.(obname).(propsName).name=...
                                              PropertyInfo(PropIndex).Name;
                                index.(obname).(propsName).datatype=...
                                          PropertyInfo(PropIndex).DataType;
                                index.(obname).(propsName).cnt=1;
                                %Pre-allocation
                                index.(obname).(propsName).value=...
                                                            cell(nsamps,1);
                                %Pre-allocation
                                index.(obname).(propsName).samples=...
                                                           zeros(nsamps,1);
                                ...
                                index.(obname).(propsName).samples(1)=...
                                                                    nsamps;
                            end
                        end
                        ...
                        if propsDataType==68 %Timestamp
                            ...
                            if kTocBigEndian
                                ...
                                tsec=fread(fid,1,'uint64','b')+...
                                     fread(fid,1,'uint64','b')/2^64;
                            else
                                %time since Jan-1-1904 in seconds
                                tsec=fread(fid,1,'uint64','l')/2^64+...
                                     fread(fid,1,'uint64','l');
                            end
                            %time since Jan-1-1904 in seconds
                            %tsec=fread(fid,1,'uint64',kTocEndian)/2^64+...
                            %             fread(fid,1,'uint64',kTocEndian);
                            %R. Seltzer: Not sure why '5/24' (5 hours) is 
                            % subtracted from the time value.  That's how
                            % it was coded in the original function I
                            % downloaded from MATLAB Central.  But I found
                            % it to be 1 hour too much.  So, I changed it
                            % to '4/24'.
                            %/864000 convert to days; +695422 days from
                            %  Jan-0-0000 to Jan-1-1904
                            %propsValue=tsec/86400+695422-5/24;
                            %/864000 convert to days; +695422 days from
                            %  Jan-0-0000 to Jan-1-1904
                            propsValue=tsec/86400+695422-4/24;
                        else  %Numeric
                            PropertyInfo(PropIndex).DataType='Numeric';
                            matType=LV2MatlabDataType(propsDataType);
                            if strcmp(matType,'Undefined')
                                e=errordlg(sprintf(['No MATLAB data ',...
                                                    'type defined for ',...
                                                    'a Property Data ',...
                                                    'Type value of',...
                                                    '%.0f''.',...
                                                     propsDataType]),...
                                    'Undefined Property Data Type');
                                uiwait(e)
                                fclose(fid);
                                return
                            end
                            if strcmp(matType,'uint8=>char')
                                propsValue=convertToText(fread(fid,1,...
                                                      'uint8',kTocEndian));
                            else
        ...
        for kk=1:numel(objOrderList)
            ...
            if ccnt>0   %If segement has raw data
                ...
                if index.(obname).dataType==32 %Datatype is a string
                    ...
                    rawDataBytes=index.(obname).nValues(ccnt)*...
                                                   index.(obname).datasize;
                    if kTocInterleavedData
...
%         for kk=1:numel(objOrderList)
%             obname=objOrderList{kk};
%             %If segement has raw data
%             if hasRawDataInSeg(segCnt,index.(obname))
%                 %This will set the offset correctly for dec data.
%                 index.(obname).rawdataoffset(ccnt)=rawdataoffset;
%                 
%                 %Update the singleSegDataSize value for the next object
%                 if index.(obname).dataType==32 %Datatype is a string
%                     singleSegDataSize=singleSegDataSize+...
%                                            index.(obname).byteSize(ccnt);
%                 else
%                     singleSegDataSize=singleSegDataSize+...
%                                          index.(obname).nValues(ccnt)*...
...
%         
        ...
        %Multiple appended raw segement (offset did not grow to be greater
        % than the DataLength)
        if (singleSegDataSize~=SegInfo.DataLength(segCnt))
            ...
            for kk=1:numel(objOrderList)   %Loop through all the objects
                ...
                if ccnt>0   %If segement has raw data
                    if kTocInterleavedData
                        %Datatype is a string
                        if index.(obname).dataType==32
                            error(['Interleaved string channels ',...
                                'are not supported.'])
                        end
                        index.(obname).multiplier(ccnt)=numAppSegs*...
                                              index.(obname).nValues(ccnt);
                        index.(obname).skip(ccnt)=singleSegDataSize/...
                                          index.(obname).nValues(ccnt)-...
                                          index.(obname).datasize;
                        index.(obname).nValues(ccnt)=1;
                    else  %Decimated Data
            ...
            if kTocInterleavedData
                ...
                for kk=1:numel(objOrderList)
                    ...
                    ccnt=hasRawDataInSeg(segCnt,index.(obname));
                    if ccnt>0  %If segement has raw data
                        %If the object has rawdata in the current segment
                        if (index.(obname).index(ccnt)==segCnt)
                            %Datatype is a string
                            if index.(obname).dataType==32
                               error(['Interleaved string channels ',...
                                      'are not supported.'])  
                            end
                            index.(obname).multiplier(ccnt)=...
                                             index.(obname).nValues(ccnt);
                            index.(obname).skip(ccnt)=singleSegDataSize/...
                                          index.(obname).nValues(ccnt)-...
                                          index.(obname).datasize;
...
%         %If the offset grew to be greater than the DataLength
%         % (from Meta Data)
            ...
            if nvals>0  %If there is data in this segement
                ...
                switch id.dataType
                    ...
                    case 32		%String
                        %From the National Instruments web page
                        % (http://zone.ni.com/devzone/cda/tut/p/id/5696)
                        % under the 'Raw Data' description on page 4:
                        %String type channels are preprocessed for fast
                        % random access. All strings are concatenated to a
                        %contiguous piece of memory. The offset of the
                        % first character of each string in this contiguous
                        % piece of memory is stored to an array of unsigned
                        % 32-bit integers. This array of offset values is
                        % stored first, followed by the concatenated string
                        % values. This layout allows client applications to
                        % access any string value from anywhere in the file
                        % by repositioning the file pointer a maximum of
                        % three times and without reading any data that is
                        % not needed by the client.
                        %Pre-allocation
                        data=cell(1,nvals*id.multiplier(rr));
                        for mm=1:id.multiplier(rr)
                            ...
                            for dcnt=1:nvals
                                ...
                                data{1,dcnt+(mm-1)*nvals}=...
                                           char(convertToText(fread(fid,...
                                           StrLength,'uint8=>char','l'))');
                            end
                    ...
                    case 68		%Timestamp
                        ...
                        for mm=1:id.multiplier(rr)
                            ...
                            data((mm-1)*nvals+1:(mm)*nvals)=tsec/86400+...
                                                               695422-4/24;
                            fseek(fid,id.skip(rr),'cof');
                        end
                        %{
						for dcnt=1:nvals
                            %time since Jan-1-1904 in seconds
							tsec=fread(fid,1,'uint64')/2^64+fread(fid,1,...
                                                                 'uint64');
							%R. Seltzer: Not sure why '5/24' (5 hours) is
                            % subtracted from the time value.  That's how
                            % it was coded in the original function I
                            ...
                            %/864000 convert to days; +695422 days from
                            % Jan-0-0000 to Jan-1-1904
							data(1,dcnt)=tsec/86400+695422-4/24;
						end
                        %}
                        cnt=nvals*id.multiplier(rr);
                    ...
                    otherwise	%Numeric
                        ...
                        %Bad Data types catch
                        if strcmp(matType,'Undefined')
                            e=errordlg(sprintf(['No MATLAB data type ',...
                                'defined for a Raw Data Type value of ',...
                                '%.0f''.', id.dataType]),...
                                'Undefined Raw Data Type');
                            uiwait(e)
                            fclose(fid);
                            return
                        end
                        if (id.skip(rr)>0)
                            ...
                            [data,cnt]=fread(fid,nvals*...
                                             id.multiplier(rr),ntype,...
                                             id.skip(rr),kTocEndian);
                            if strcmp(matType,'uint8=>char')
                          ...
                          % Added by Haench start
                            % complex CDB data
                            if  (id.dataType == 524300) || ...
                                                   (id.dataType == 1048589)
                                [data,cnt]=fread(fid,2*nvals*...
                                     id.multiplier(rr),matType,kTocEndian);
                            ...
                            % Haench end
                            % Original: [data,cnt]=fread(fid,nvals*...
                            %        id.multiplier(rr),matType,kTocEndian);
                        end
                end
...
%Re-organize the 'ob' structure into a more user friendly format for
% output.
...
for i=1:numel(obFieldNames)
    ...
    if strcmp(index.(cname).long_name,'Root')
        ...
        if isfield(index.(cname),'PropertyInfo')
            for p=1:numel(index.(cname).PropertyInfo)
                ...
                DataStructure.Root.Property(p).Name=...
                                        index.(cname).PropertyInfo(p).Name;
                ...
                switch DataType
                    ...
                    case 'Time'
                        ...
                        if index.(cname).(cfield).cnt==1
                            if iscell(index.(cname).(cfield).value)
                                Value=datestr(cell2mat(...
                                         index.(cname).(cfield).value),...
                    ...
                    case 'Numeric'
                        if isfield(index.(cname).(cfield),'cnt')
                            ...
                            if iscell(index.(cname).(cfield).value)
                                Value=NaN(numel(cell2mat(...
                                    index.(cname).(cfield).value)),1);
                            else
                                Value=NaN(numel(...
                                    index.(cname).(cfield).value),1);
                            end
                    ...
                    otherwise
                        e=errordlg(sprintf(['No format defined for ',...
                            'Data Type %s in the private function ',...
                            '"postProcess" within %s.m. '],...
                            index.(cname).(cfield).datatype,mfilename),...
                            'Undefined Property Data Type');
                        uiwait(e)
                        return
                end
    ...
    if (isfield(ob,cname))
        if isfield(ob.(cname),'data')
            ...
            %The following field is redundant because the information can
            % be obtained from the size of the 'Data' field.
            DataStructure.MeasuredData(cntData).Total_Samples=...
                                                       ob.(cname).nsamples;
        else
    ...
    if isfield(index.(cname),'PropertyInfo')
        for p=1:numel(index.(cname).PropertyInfo)
            cfield=index.(cname).PropertyInfo(p).FieldName;
            DataStructure.MeasuredData(cntData).Property(p).Name=...
                                               index.(cname).(cfield).name;
...
switch(LVType)
    ...
    case {2,6}
        sz=2;
    case 32
        e=errordlg(['Do not call the getDataSize function for strings.',...
            ' Their size is written in the data file','Error']);
        uiwait(e)
        sz=NaN;
    case 11
        sz=10;
...
%Convert numeric bytes to the character encoding localy set in MATLAB
% (TDMS uses UTF-8)
